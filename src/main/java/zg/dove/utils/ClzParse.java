...
public class ClzParse {
     ...
     */
    public static String[] findClassesByPackage(String packageName) throws Exception {
        return ClzParse.findClassesByPackage(packageName, EMPTY_LIST, EMPTY_LIST);
    }
    ...
    public static String[] findClassesByPackage(String packageName, List<String> included, List<String> excluded) throws Exception {
        ...
        while (dirs.hasMoreElements()) {
            ...
            if ("file".equals(protocol)) {
                ClzParse.findClassesByDirPackage(packageOnly, included, excluded, URLDecoder.decode(url.getFile(), "UTF-8"),
                        recursive, vResult);
            }
                    ...
                    if (name.startsWith(packageDirName)) {
                        ...
                        if ((idx != -1) || recursive) {
                            if (name.endsWith(".class") && !entry.isDirectory()) {
                                ...
                                ClzParse.includeOrExcludeClass(packageName, className, included, excluded, vResult);
                            }
                        }
                    }
                }
     ...
     * @param classes 结果集
     */
    private static void findClassesByDirPackage(String packageName, List<String> included, List<String> excluded,
        ...
        for (File file : dirfiles) {
            if (file.isDirectory()) {
                // 递归处理
                ClzParse.findClassesByDirPackage(packageName + "." + file.getName(), included, excluded, file.getAbsolutePath(),
                        recursive, classes);
            } else {
                String className = file.getName().substring(0, file.getName().length() - 6);
                ClzParse.includeOrExcludeClass(packageName, className, included, excluded, classes);
            }
        }
    }
    ...
    private static void includeOrExcludeClass(String packageName, String className, List<String> included,
        ...
        if (ClzParse.isIncluded(className, included, excluded)) {
    ...
    private static boolean isIncluded(String name, List<String> included, List<String> excluded) {
        ...
        if (included.size() == 0 && excluded.size() == 0) {
            ...
            boolean isIncluded = ClzParse.find(name, included);
            boolean isExcluded = ClzParse.find(name, excluded);
